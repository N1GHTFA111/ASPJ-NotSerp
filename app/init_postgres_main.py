...
# cd app
# flask --app app/init_postgres db init
# flask --app app/init_postgres db migrate
# flask --app init_postgres db upgrade
# flask --app init_postgres db downgrade

app = Flask(__name__,
            static_folder='../static',  # Set the new path for the static folder
...
class UserModel(db.Model, UserMixin):
    ...
    # Add cascade option to delete cart items when UserModel is deleted
    carts = db.relationship('CartModel', backref='user_model', cascade='all, delete-orphan')

    # if user delete, delete all associated blogs
    blogs = db.relationship('BlogModel', backref='user_model', cascade='all, delete-orphan')

    # vouchercart cascade delete
    # Add cascade option to delete VoucherCartModel when UserModel is deleted
    uservouchercarts = db.relationship('VoucherCartModel', backref='user_model', cascade='all, delete-orphan')
...
class FeedbackModel(db.Model):
    ...
    def __init__(self, user_who_created, title, description):
        ...
        self.username = user_who_created.get_username()
...
def get_feedback_made_by_user(user_who_created):
    all_feedback_made_by_user = db.session.execute(
        db.Select(FeedbackModel).filter_by(user_id=user_who_created.get_id())).scalars()
...
class EVIRECModel(db.Model):
    ...
    user_who_added = db.Column(db.String(100), nullable=False)
    time = db.Column(db.DateTime, nullable=False)
...
class InventoryModel(db.Model):
    ...
    carts = relationship('CartModel', backref='product_from_inventory', lazy=True, cascade='all, delete-orphan')
...
class PointsModel(db.Model):
    code_id = db.Column(db.String(100), primary_key=True)
    time = db.Column(db.DateTime, nullable=False)
    points = db.Column(db.Integer, nullable=False)
    redemption_code = db.Column(db.String(6), nullable=False)
    expiration_date = db.Column(db.DateTime, nullable=False)
    claimedBy = db.Column(db.String(200), nullable=True)

    def __init__(self, points):
        current_time = datetime.datetime.now()
        self.code_id = Sentinel.generate_code_id()
        self.time = current_time
        self.points = points
        self.redemption_code = self.unique_code()

        # Expiration is automatically 7 days from generation date.
        self.expiration_date = (current_time + timedelta(days=7))

    def unique_code(self):
        alphabet = string.ascii_letters + string.digits
        while True:
            alphanumeric_code = ''.join(secrets.choice(alphabet) for _ in range(6))
            existing_codes = PointsModel.query.filter_by(redemption_code=alphanumeric_code).all()

            if not existing_codes or all(record.claimedBy for record in existing_codes):
                return alphanumeric_code

    def set_claimed_by(self, value):
        self.claimedBy = value


##################
# database of valid codes auto populate
def populate_demo_valid_points_code():
    for i in range(10):
        new_code = PointsModel(300)
        db.session.add(new_code)

    db.session.commit()


# this will store all redeemed vouchers
# voucher transaction model
class VouchersRedeemedModel(db.Model):
    time = db.Column(db.DateTime, nullable=False)
    voucher_transaction_id = db.Column(db.String(200), primary_key=True)
    voucher_cart_id = db.Column(db.String(200), nullable=False)
    redeemedBy = db.Column(db.String(100), nullable=False)

    def __init__(self, voucher_cart_id, redeemer):
        self.time = datetime.datetime.now()
        self.voucher_transaction_id = Sentinel.generate_voucher_transaction_id()
        self.voucher_cart_id = voucher_cart_id
        self.redeemedBy = redeemer.user_id
...
class VoucherCartModel(db.Model):
    ...
    voucher_inventory_voucher_id = db.Column(db.String(100), db.ForeignKey('voucher_inventory_model.voucher_id'),
                                             nullable=False)

    # e.g. VoucherCartModel = VoucherCartModel.voucher_inventory
    voucher_inventory = relationship("VoucherInventoryModel",
                                     backref=backref("voucher_from_voucher_cart", uselist=False), lazy=True,
...
def update_voucher_blueprint_admin(voucher_blueprint_id, name, desc, percent, quantity, points_needed):
    voucher_to_update = db.session.execute(
        db.Select(VoucherInventoryModel).filter_by(voucher_id=voucher_blueprint_id)).scalar_one()
...
def delete_voucher_blueprint_admin(voucher_blueprint_id):
    voucher_to_delete = db.session.execute(
        db.Select(VoucherInventoryModel).filter_by(voucher_id=voucher_blueprint_id)).scalar_one()
...
def check_valid_code(code):
    for valid_code in PointsModel.query.all():
        if code == valid_code.redemption_code:
            return valid_code.points
...
def redeem_code_for_points_helper(code, curr_user):
    points = check_valid_code(code)
    if check_valid_code(code) is not None:
        # find the exact point code with that code
        code_obj = db.session.execute(db.Select(PointsModel).filter_by(redemption_code=code)).scalar_one()
        # update the claim by
        code_obj.set_claimed_by(curr_user.get_id())
        db.session.commit()

        # get the valid code
        redeem_code_for_points(curr_user, points)
    return


def generate_voucher_user(voucher_blueprint_id, user_obj):
    voucher_to_create = db.session.execute(
        db.Select(VoucherInventoryModel).filter_by(voucher_id=voucher_blueprint_id)).scalar_one()

    if check_sufficient_points(user_obj, voucher_to_create.get_unit_points_needed()):
        print("Here")
        voucher_to_decrease_quantity = db.session.execute(
            db.Select(VoucherInventoryModel).filter_by(voucher_id=voucher_to_create.get_voucher_id())).scalar_one()
...
def get_all_user_voucher_can_redeem():
    all_user_vouchers = db.session.execute(
        db.Select(VoucherInventoryModel).filter(VoucherInventoryModel.quantity > 0)).scalars()
...
def redeem_voucher(stripe_code, user_that_redeemed):
    try:
        voucher_to_use = db.session.execute(
            db.Select(VoucherCartModel).filter_by(coupon_stripe_id=stripe_code)).scalar_one()
        ...
        # add the voucher redeem transaction
        new_voucher_redeem = VouchersRedeemedModel(voucher_cart_id=voucher_to_use.get_voucher_cart_id(),
                                                   redeemer=user_that_redeemed)
        db.session.add(new_voucher_redeem)
        db.session.commit()
...
def create_product(product_id, product_name, description, quantity, unit_price,
    ...
    add_to_log(classification="JOB",
               target_route=request.url,
               priority=0,
               details=f"Product of id {product_id} created by Admin with id of {flask_login.current_user.get_id()}"
               )
    db.session.commit()
...
def update_product_helper(product_id, product_name, description, quantity, unit_price,
    ...
    add_to_log(classification="JOB",
               target_route=request.url,
               priority=0,
               details=f"Product of id {product_id} updated by Admin with id of {flask_login.current_user.get_id()}"
               )
...
def delete_product_helper(prod_id):
    ...
    add_to_log(classification="JOB",
               target_route=request.url,
               priority=0,
               details=f"Product of id {prod_id} deleted by Admin with id of {flask_login.current_user.get_id()}"
               )
    db.session.commit()
...
def registerUser():
    ...
    if request.method == "POST" and createuserform.validate():
        ...
        if file.filename:
            ...
            if Sentinel.FileChecker.is_file_safe(file_to_test, file_name):
                ...
                file.save(app.static_folder + '/profile_pics/' + new_file_name)
    ...
    # if validation failed
        ...
        add_to_log(classification="FAILED REGISTRATION",
                   target_route=html.escape(request.url),
                   priority=1,
                   details=f"Failed user registration with parameters:\n "
                           f"Username: {createuserform.username.data}\n"
                           f"Email: {createuserform.email.data}\n"
                           f"Phone: {createuserform.phone.data}\n")
...
def registerAdmin():
    ...
    # if validation failed
        ...
        add_to_log(classification="FAILED REGISTRATION",
                   ...
                   priority=1,
                   details=f"Failed user registration with parameters:\n "
                           f"Username: {createadminform.username.data}\n"
                           f"Email: {createadminform.email.data}\n"
                           f"Phone: {createadminform.phone.data}\n")
    else:
...
def login():
    ...
    if request.method == "POST" and createloginform.validate():
        ...
        if not check_user_exist:
            ...
            add_to_log(classification="FAILED LOGIN",
                       target_route=html.escape(request.url),
                       priority=1,
                       details=f"Failed login with Email:{email}")
        ...
        if check_user_exist and bcrypt.checkpw(password,
            ...
            if check_user_exist.get_enable_2fa_email() == "Not Enabled":
                ...
                return redirect(
                    url_for('confirm_2fa_login', user_id=check_user_exist.get_id(), email=check_user_exist.get_email(),
            ...
            add_to_log(classification="FAILED LOGIN",
                       target_route=html.escape(request.url),
                       priority=1,
                       details=f"Failed user login with parameter:\n "
                               f"Email: {createloginform.email.data}\n"
                       )
...
def forget_password():
    ...
    if request.method == "POST" and email_verification_form.validate():
        ...
        otp_token = totp_for_forget_password.now()
...
def deleteUser():
    ...
    try:
        ...
        if current_user_to_delete.get_profile_pic() != 'default.jpg':
            path = os.path.join(app.static_folder, 'profile_pics', current_user_to_delete.get_profile_pic())
            os.remove(path)

        add_to_log(classification="JOB",
                   target_route=html.escape(request.url),
                   priority=0,
                   details=f"User with id of {current_user_to_delete.get_id()} deleted")
...
def export_data():
    ...
    file_path = os.path.join(os.path.dirname(app.root_path), 'static', 'log_reports', excel_file)
    df.to_excel(file_path, index=False)

    add_to_log(classification="JOB",
               target_route=html.escape(request.url),
               priority=0,
               details=f"Exported log to path: {file_path}")
...
def serve_log_file(filename):
    add_to_log(classification="JOB",
               target_route=html.escape(request.url),
               priority=0,
               details=f"Downloaded log file: {filename}")
...
def get_admin_dashboard(username):
    ...
    if username == flask_login.current_user.get_username():
        ...
        start_date = logsmodel[0].get_time()
        end_date = logsmodel[-1].get_time()

        # slider auto give u the first time closest to the filter
...
def get_users_per_page(page, per_page):
    usermodel = UserModel.query.paginate(page=page, per_page=per_page)
    return usermodel
...
def get_admin_user_dashboard(username):
    ...
    if username == flask_login.current_user.get_username():
        ...
        page = request.args.get('page', 1, type=int)
        per_page = 5  # Set the number of items per page as desired
        users_paginated = get_users_per_page(page, per_page)

        pagination = Pagination(page=page, total=users_paginated.total, per_page=5, css_framework='bootstrap5')
        ...
        return render_template("dashboard_admin_usermanage.html", profile_pic_name=profile_pic_name, username=username,
                               usermodel=users_paginated, pagination=pagination)
    else:
        add_to_log(classification="JOB",
                   target_route=html.escape(request.url),
                   priority=2,
    ...
    if request.method == "POST" and createroleform.validate_on_submit():
        ...
        try:
            ...
            add_to_log(classification="JOB",
                       target_route=html.escape(request.url),
                       priority=0,
                       details=f"Role created with name: {role_name}")
        except IntegrityError:
            db.session.rollback()

    # did not pass validation
    elif request.method == "POST":
        add_to_log(classification="FAILED CREATE ROLE",
                   target_route=html.escape(request.url),
                   priority=2,
                   details=f"Role created with name: {createroleform.rolename.data}")
...
def updateRole_Admin(username):
    ...
    if request.method == "POST" and updateroleform.validate_on_submit():
        ...
        add_to_log(classification="JOB",
                   target_route=html.escape(request.url),
                   priority=2,
                   details=f"Role created with name: {updateroleform.rolename.data}")

        return redirect(
            url_for('get_admin_roles_dashboard', profile_pic_name=current_user.get_profile_pic(), username=username))

    elif request.method == "POST":
        add_to_log(classification="FAILED UPDATE ROLE",
                   target_route=html.escape(request.url),
                   priority=2,
                   details=f"Role created with name: {updateroleform.rolename.data}")
        return redirect(
...
def deleteRole_Admin(username):
    ...
    if request.method == "POST":
        ...
        add_to_log(classification="JOB",
                   target_route=html.escape(request.url),
                   priority=2,
                   details=f"Role deleted with id: {role_id}")
...
def updateUser_admin(email):
    ...
    if request.method == "POST" and updateuserform.validate():
        ...
        add_to_log(classification="FAILED UPDATE USER",
                   target_route=html.escape(request.url),
                   priority=2,
                   details=f"Admin with user id of {flask_login.current_user.get_id()} failed updated user profile with email of {email}.")
        return redirect(url_for('get_admin_dashboard', username=flask_login.current_user.get_username(),
                                logged_in=flask_login.current_user.is_authenticated,
                                profile_pic_name=flask_login.current_user.get_profile_pic()))
    else:
...
############################################################
# user side of transaction
@app.route('/dashboard/<path:username>/finance', methods=['GET', 'POST'])
@login_required
@roles_required('USER')
def get_user_finance_dashboard(username):
    profile_pic_name = request.args.get('profile_pic_name')
    if username == flask_login.current_user.get_username():
        current_user = username
        print(current_user)
        ...
        transactions = db.session.execute(
            db.Select(TransactionModel).filter_by(account_id=flask_login.current_user.account.get_id())).scalars()
        # i want to show only codes that the user claimed for specific user
        # codes = PointsModel.query.all()
        codes = db.session.execute(
            db.Select(PointsModel).filter_by(claimedBy=flask_login.current_user.get_id())).scalars()
        serialized_codes = [
            {
        ...
        # vouchers_redeemed = VouchersRedeemedModel.query.all()
        vouchers_redeemed = db.session.execute(
            db.Select(VouchersRedeemedModel).filter_by(redeemedBy=flask_login.current_user.get_id())).scalars()
        ...
        return render_template("dashboard_user_transactions.html", profile_pic_name=profile_pic_name, username=username,
                               transactions=transactions, codes=serialized_codes, vouchers_redeemed=vouchers_redeemed)
    else:
        return redirect(url_for('get_dashboard', username=flask_login.current_user.get_username(),
                                profile_pic_name=flask_login.current_user.get_profile_pic()))
...
def get_admin_finance_dashboard(username):
    ...
    if username == flask_login.current_user.get_username():
        ...
        transactions = [
            {
                ...
                'total_price': transaction.unit_price * transaction.product_quantity
            }
            for transaction in TransactionModel.query.all()
        ]
        codes = [
            {
                'code_id': code.code_id,
            ...
            for code in PointsModel.query.all()
        ]
        vouchers_redeemed = [
            {
                'voucher_transaction_id': voucher.voucher_transaction_id,
                'time': voucher.time,
                'voucher_cart_id': voucher.voucher_cart_id,
                'redeemedBy': voucher.redeemedBy
            }
            for voucher in VouchersRedeemedModel.query.all()
        ]
        ...
        return render_template("dashboard_admin_finance.html", profile_pic_name=profile_pic_name, username=username,
                               transactions=transactions, codes=codes, vouchers_redeemed=vouchers_redeemed)
    else:
        return redirect(url_for('get_dashboard', username=flask_login.current_user.get_username(),
                                profile_pic_name=flask_login.current_user.get_profile_pic()))


# to populate valid codes

@app.route('/populate_valid_codes', methods=['GET', 'POST'])
def populate_valid_codes():
    populate_demo_valid_points_code()
    return redirect(url_for('index'))
...
def checkout_finalise():
    ...
    if one_time_pass.get_expiration_date() > datetime.datetime.now():
        ...
        if discountcode != None and discountcode != "":
            redeem_voucher(discountcode, flask_login.current_user)
...
@limiter.limit("1000/hour", methods=["POST"])  # apply rate limiting to login route
@login_required
@roles_required('PRODUCT_ADMIN')
    ...
    if request.method == "POST":
        ...
        if file.filename:
            ...
            if Sentinel.FileChecker.is_file_safe(file_to_test, file_name):
                ...
                file.save(app.static_folder + '/product_pics/' + new_file_name)
            else:
                add_to_log("SUSPICIOUS FILE UPLOAD", request.url, 2, f"Suspicious File rejected: {file.filename}")
                new_file_name = "default.jpg"
        else:
...
def delete_product(username):
    ...
    if request.method == "POST":
        ...
        if product_to_delete.get_product_pic() != 'default.jpg':
            path = os.path.join(app.static_folder, 'product_pics', product_to_delete.get_product_pic())
            os.remove(path)
...
def create_new_evirec_path_helper(logidlist, pathname, desc):
    for id in logidlist:
        ...
        add_to_log(classification='JOB',
                   target_route=html.escape(request.url),
                   priority=0,
                   details=f"Admin with user id of {flask_login.current_user.get_id()} created EVIREC PATH of name {pathname}.")
    ...
    for id in logidlist:
        if id not in current_id:
            ...
            add_to_log(classification='JOB',
                       target_route=html.escape(request.url),
                       priority=0,
                       details=f"Admin with user id of {flask_login.current_user.get_id()} updated EVIREC PATH of name {pathname}.")
    db.session.commit()
...
def update_evirec_path_name_helper(old_path, new_pathname):
    ...
    for entry in list_of_pathname_evirec:
        ...
        add_to_log(classification='JOB',
                   target_route=html.escape(request.url),
                   priority=0,
                   details=f"Admin with user id of {flask_login.current_user.get_id()} updated EVIREC PATH of name {new_pathname}.")
    db.session.commit()
...
def delete_evirec_path_helper(pathName):
    ...
    for entry in list_of_pathname_evirec:
        ...
        add_to_log(classification='JOB',
                   target_route=html.escape(request.url),
                   priority=0,
                   details=f"Admin with user id of {flask_login.current_user.get_id()} deleted EVIREC PATH of name {pathName}.")
    db.session.commit()
...
def delete_evirec_item_helper(evirec_id):
    ...
    add_to_log(classification='JOB',
               target_route=html.escape(request.url),
               priority=0,
               details=f"Admin with user id of {flask_login.current_user.get_id()} deleted EVIREC ITEM of id {entry_to_delete.evirec_id}.")
    ...
    if username == flask_login.current_user.get_username() and createform.validate_on_submit():
        ...
        if file.filename:
            ...
            if Sentinel.FileChecker.is_file_safe(file_to_test, file_name):
                ...
                # new_file_name = secrets.randbits(32) + "." + extension
                new_file_name = Sentinel.generate_secure_filename(extension)
                file.save(app.static_folder + '/blog_pics/' + new_file_name)
            else:
                add_to_log("SUSPICIOUS FILE UPLOAD", request.url, 2, f"Suspicious File rejected: {file.filename}")
                new_file_name = "default.jpg"
        else:
...
def admin_update_blog(username):
    ...
    if username == flask_login.current_user.get_username() and updateform.validate_on_submit():
        ...
        if file.filename:
            ...
            blog_update = BlogModel.query.filter_by(id=blog_id).first()
            new_file_name = blog_update.get_picture_name()
        update_blog_post_helper(blog_id=blog_id, title=title, description=description, picture_name=new_file_name,
                                user_who_updated=user_who_updated)
...
def admin_view_all_vouchers(username):
    ...
    if username == flask_login.current_user.get_username():
        ...
        return render_template("dashboard_admin_voucher.html", username=flask_login.current_user.get_username(),
                               profile_pic_name=flask_login.current_user.get_profile_pic(),
                               voucherinventorymodel=voucherinventorydb,
                               updateform=updateform, createform=createform)
    else:
        return redirect(url_for('authenticated_user'))
...
def user_view_all_vouchers(username):
    ...
    if username == flask_login.current_user.get_username():
        ...
        return render_template("dashboard_user_voucher.html", username=flask_login.current_user.get_username(),
                               profile_pic_name=flask_login.current_user.get_profile_pic(),
                               voucherinventorymodel=vouchers_to_redeem,
...
def user_add_points(username):
    ...
    if username == flask_login.current_user.get_username():
        ...
        redeem_code_for_points_helper(code=code, curr_user=curr_user)
        print("Success")
